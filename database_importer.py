...
class DatabaseImporter:
    ...
    # @staticmethod
    # def apply_scd_type2(engine, staging_table_name, dw_table_name, key_column, columns_to_update):
    #     from datetime import datetime
    #     metadata = MetaData()
    #
    #     # Reflect the tables
    #     staging_table = Table(staging_table_name, metadata, autoload_with=engine)
    #     dw_table = Table(dw_table_name, metadata, autoload_with=engine)
    #
    #     with engine.connect() as connection:
    #         staging_records = connection.execute(select(staging_table)).fetchall()
    #         for record in staging_records:
    #             dw_record = connection.execute(
    #                 select(dw_table).where(dw_table.columns[key_column] == record[key_column])).first()
    #
    #             if dw_record is not None:
    #                 if any(dw_record[column] != record[column] for column in columns_to_update):
    #                     # Update the existing record's end date and flag
    #                     update_stmt = (
    #                         dw_table.update().where(dw_table.columns[key_column] == record[key_column])
    #                         .where(dw_table.columns['IsCurrent'])
    #                         .values(IsCurrent=False, EndDate=datetime.now())
    #                     )
    #                     connection.execute(update_stmt)
    #
    #                     # Insert new record
    #                     new_record = record.copy()
    #                     new_record['StartDate'] = datetime.now()
    #                     new_record['EndDate'] = datetime.max
    #                     new_record['IsCurrent'] = True
    #                     connection.execute(dw_table.insert().values(new_record))
    #             else:
    #                 # Insert new record as it does not exist
    #                 new_record = record.copy()
    #                 new_record['StartDate'] = datetime.now()
    #                 new_record['EndDate'] = datetime.max
    #                 new_record['IsCurrent'] = True
